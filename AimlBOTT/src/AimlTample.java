...
public class AimlTample {
	...
	//	 aiml =new File("E:/Programowanie/KWD/6sem/ab/bots/test/aiml/test.aiml");
	//	 File csv = new File("E:/Programowanie/KWD/6sem/ab/bots/test/aimlif/test.aiml.csv");
	...
	void addaiml(String question , String answer) throws IOException // jezeli jest pelny
	{	
		...
		{	String next = plikaiml.nextLine();
			...
			if(temps == lastIndexOfQuestion)
			{
				...
				tempik +="<template>"+delimiter+" "+answer+""+delimiter+" </template>"+delimiter+" </category>"+delimiter+"";
				break;
		...
		pwaiml.write(tempik);
		pwcsv.write(""+delimiter+""+(lastIndexOfQuestion+1)+","+question.toUpperCase()+",*,*,"+answer+","+aimlName+"");
		pwaiml.close();
		pwcsv.close();
	}
}
